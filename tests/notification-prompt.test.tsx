...
vi.mock("@/lib/notificationRuntime", () => ({
  getNotificationRuntimePolicy: () => ({ enabled: runtime.enabled, statusKey: "notification-test-status" }),
...
describe("notification runtime lifecycle", () => {
  ...
  it("keeps one listener while token registration and navigation change UI state", async () => {
    ...
    runtime.requestToken.mockImplementationOnce(() => new Promise<string>((resolve) => { finish = resolve; }));
    ...
    await act(async () => { finish("synthetic-fcm-token"); });
  ...
  it("unsubscribes an asynchronous listener that finishes after cleanup", async () => {
    ...
    runtime.listen.mockImplementationOnce(() => new Promise<() => void>((resolve) => { finish = resolve; }));
