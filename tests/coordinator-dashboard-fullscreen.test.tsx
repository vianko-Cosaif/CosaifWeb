...
import RailQueueBoard from "@/features/rail-queue/coordinador/RailQueueBoard";

vi.mock("@/features/rail-queue/useRealtimeBoardRefresh", () => ({ useRealtimeBoardRefresh: () => "connected" }));
vi.mock("@/lib/notificationSound", () => ({ preloadNotificationSound: vi.fn(), primeNotificationSound: vi.fn().mockResolvedValue(false), playNotificationSound: vi.fn() }));
...
async function renderDashboard() {
  const view = render(<RailQueueBoard localidadId={2} />);
  await screen.findAllByText("0033");
  const board = view.container.querySelector<HTMLElement>('[data-coordinator-dashboard="true"]');
  expect(board).not.toBeNull();
  return board!;
}

describe("coordinator dashboard fullscreen", () => {
  it("expands the dashboard itself and exits without resetting its queue", async () => {
