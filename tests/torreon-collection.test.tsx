...
describe("Torreón collection lifecycle", () => {
  it("aborts the old scope and never shows its late response under another company", async () => {
    ...
    const hook = renderHook(({ key, fetchRows }) => useTorreonCollection({ queryKey: key, fetchRows }), {
  ...
  it("shares an in-flight initial load with ready/resume, while grouping forced updates into one subsequent read", async () => {
    ...
    act(() => { void hook.result.current.load(); void hook.result.current.load(); });
    expect(fetchRows).toHaveBeenCalledTimes(1);
    act(() => { void hook.result.current.load(true); void hook.result.current.load(true); });
  ...
  it("keeps the last successful rows and exposes a refresh failure instead of an empty operation", async () => {
    const fetchRows = vi.fn().mockResolvedValueOnce([8]).mockRejectedValueOnce(new Error("Servicio no disponible")).mockResolvedValueOnce([9]);
    const hook = renderHook(() => useTorreonCollection<number>({ queryKey: "scope", fetchRows }));
  ...
  it("hides old rows while another scope loads and does not preserve them if that scope fails", async () => {
    ...
    const hook = renderHook(({ key, fetchRows }) => useTorreonCollection<number>({ queryKey: key, fetchRows }), {
  ...
  it("aborts collection and detail reads on unmount", async () => {
    ...
    const fetchRows = vi.fn((signal: AbortSignal) => { listSignal = signal; return pendingList.promise; });
    ...
    const second = renderHook(() => useTorreonCollection<number>({ queryKey: "scope", fetchRows: loadedRows }));
    await waitFor(() => expect(second.result.current.rows).toEqual([1]));
    act(() => { void second.result.current.refreshItem(1, (signal) => { itemSignal = signal; return pendingItem.promise; }, (_, value) => [value]); });
    second.unmount();
  ...
  it("runs item refresh after a pending collection so the collection cannot overwrite a newer item", async () => {
    ...
    const hook = renderHook(() => useTorreonCollection<number>({ queryKey: "scope", fetchRows }));
    act(() => { void hook.result.current.refreshItem(1, fetchItem, (_, item) => [item]); });
  ...
  it("does not overwrite a realtime update with a collection response already in flight", async () => {
    ...
    const fetchRows = vi.fn().mockResolvedValueOnce([1]).mockReturnValueOnce(stale.promise).mockReturnValueOnce(fresh.promise);
    const hook = renderHook(() => useTorreonCollection<number>({ queryKey: "scope", fetchRows }));
    await waitFor(() => expect(hook.result.current.rows).toEqual([1]));
    act(() => { void hook.result.current.load(true); });
  ...
  it("does not fetch or retain rows when scope is disabled", async () => {
    ...
    const hook = renderHook(({ enabled }) => useTorreonCollection<number>({ queryKey: "scope", fetchRows, enabled }), { initialProps: { enabled: true } });
    await waitFor(() => expect(hook.result.current.rows).toEqual([1]));
