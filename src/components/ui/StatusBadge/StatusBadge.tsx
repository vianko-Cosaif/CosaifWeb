...
export function getStatusTone(status?: string | null): StatusTone {
  const normalized = String(status || "").trim().toUpperCase();
  if (["CONCLUIDO", "COMPLETADO", "RESUELTO", "ACTIVO"].includes(normalized)) {
    return "success";
  }
  if (["EN_PROCESO", "PROCESO", "INICIADO"].includes(normalized)) {
    return "info";
  }
  if (["DETENIDO", "BLOQUEADO", "ESPERA", "PENDIENTE", "SOLICITADO", "ABIERTO"].includes(normalized)) {
    return "warning";
  }
  if (["CANCELADO", "ERROR", "RECHAZADO"].includes(normalized)) {
    return "danger";
  }
...
export default function StatusBadge({
  ...
  return (
    <span
      className={cn(
        "inline-flex w-fit items-center rounded-md border font-semibold leading-5",
