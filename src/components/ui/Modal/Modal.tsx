...
export default function Modal({
  ...
  return createPortal(
    <div
      ref={overlay}
      className="fixed inset-0 z-[150] flex items-end justify-center bg-black/40 p-2 backdrop-blur-sm sm:items-center sm:p-4"
      onMouseDown={(event) => {
    ...
    >
      <div
        ...
        tabIndex={-1}
        className={cn(
          "relative max-h-[92dvh] w-full overflow-y-auto overscroll-contain rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] shadow-xl outline-none",
          maxWidth,
          className,
        )}
        role="dialog"
      ...
      >
        <header className="sticky top-0 z-10 flex items-center justify-between gap-3 border-b border-[var(--app-border)] bg-[var(--app-surface)] px-4 py-3 sm:px-6">
          ...
          <button
            type="button"
            onClick={onClose}
            className="inline-flex h-11 w-11 shrink-0 items-center justify-center rounded-lg text-[var(--app-text-muted)] hover:bg-[var(--app-surface-muted)] focus-visible:ring-2 focus-visible:ring-sky-500"
