...
export async function proxy(req: NextRequest) {
  ...
  if (isProtectedTransport) {
    ...
    const requestedOwner = req.headers.get("x-cosaif-outbox-owner");
    if (requestedOwner && requestedOwner !== storageScope(session)) {
      return NextResponse.json({ error: "La solicitud pertenece a otra sesión." }, { status: 403, headers: { "Cache-Control": "no-store" } });
    }
    return NextResponse.next();
  }
