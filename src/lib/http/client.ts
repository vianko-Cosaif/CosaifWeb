...
function keyFor(url: string, init: RequestInit = {}, explicit?: string) {
  const headers = JSON.stringify(
    [...new Headers(init.headers).entries()].sort(([a], [b]) => a.localeCompare(b)),
  );
  ...
  try {
    ...
    if (timedOut)
      throw new ClientRequestError(
        "El servicio tardó demasiado. Puedes reintentar.",
        408,
        "TIMEOUT",
        true,
      );
    if (error instanceof TypeError)
      throw new ClientRequestError("No hay conexión con el servicio.", 0, "NETWORK", true);
    throw error;
  } finally {
    clearTimeout(timer);
...
export function peekCachedJson<T>(url: string, key?: string): T | undefined {
  ...
  // A read makes this the most recently used entry, so a frequently visited
  // screen is not evicted merely because it was opened early in the session.
  responseCache.delete(cacheKey);
...
export function invalidateCachedJson(match?: string | RegExp) {
  const matches = (key: string) =>
    !match ||
    (typeof match === "string" ? key.includes(match) : ((match.lastIndex = 0), match.test(key)));
...
export async function cachedFetchJson<T>(
  ...
  if (!entry) {
    ...
    next.promise = readJson<T>(url, { ...init, signal: controller.signal }, timeoutMs)
      .then((data) => {
        const ttlMs = Math.max(0, options.ttlMs ?? 20_000);
        if (ttlMs && !controller.signal.aborted) {
          responseCache.set(key, { data, expiresAt: Date.now() + ttlMs });
          for (const [oldKey, value] of responseCache)
            if (value.expiresAt <= Date.now()) responseCache.delete(oldKey);
