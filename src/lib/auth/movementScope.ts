...
export function resolveMovementReadScope(
  session: VerifiedSession,
  context: MovementReadContext,
  params: URLSearchParams,
) {
  const mode = session.authorization.scope.mode;
  const isClient = session.role === "CLIENTE" || session.role === "ARRASTRE_TORREON";
  ...
  const restrictCompany =
    !sharedCurrentLocality && (isClient || mode === "COMPANY" || mode === "COMPANY_LOCALITY");
  ...
  if (
    restrictLocality &&
    (!assignedLocalidadId || (requestedLocalidadId && requestedLocalidadId !== assignedLocalidadId))
  ) {
    throw new MovementScopeError("Solo puedes consultar movimientos de tu localidad asignada.");
  }
  if (
    restrictCompany &&
    (!assignedEmpresaId || (requestedEmpresaId && requestedEmpresaId !== assignedEmpresaId))
  ) {
...
export function recordMatchesMovementScope(
  ...
  return (
    (!scope.empresaId || positiveId(record.empresaId) === scope.empresaId) &&
...
export function scopePrivateClientMovementRead(
  session: VerifiedSession,
  path: string,
  method: string,
  params: URLSearchParams,
  ...
  if (
    !/^\/(?:movimientos|rondas|torno|torreon\/(?:movimientos|rondas|arrastres))(?:\/|$)/.test(path)
  )
    return;
  ...
  if (
    (embeddedLocality && Number(embeddedLocality[1]) !== scope.localidadId) ||
    (embeddedCompany && Number(embeddedCompany[1]) !== scope.empresaId)
  ) {
    throw new MovementScopeError(
      "Solo puedes consultar movimientos de tu empresa y localidad asignadas.",
    );
  }
