...
function IncidentEmptyState({ loading, emptyStateText, onRefresh, refreshing }: {
  ...
  return (
    <DataEmptyState
      ...
      description="Ajusta los filtros disponibles o la búsqueda, que se aplica a esta página."
...
function IncidentLocation({ row }: { row: IncidenteRow }) {
  return (
    <div className="min-w-0">
      <p className="truncate font-semibold text-slate-900 dark:text-white">{row.localidad || row.fuente || "Sin localidad"}</p>
...
function IncidentesTableComp({
  ...
  return (
    <section className="flex flex-col gap-4 p-4 sm:p-5" aria-busy={loading}>
      {!isDesktop ? (
      <div>
            ...
            {deferredData.map((row, index) => (
              ...
              >
                <button
                  ...
                  className={cn(
                    "w-full rounded-lg border border-[var(--app-border)] bg-[var(--app-surface)] p-4 text-left shadow-sm transition-shadow hover:shadow-md",
                ...
                >
                  <div className="flex items-start justify-between gap-3">
                    <div className="min-w-0">
                      <div className="flex flex-wrap items-center gap-2">
                        <span className="font-mono text-xs font-semibold text-slate-400">#{row.id ?? "—"}</span>
                        <span className={`rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${operationTone(row)}`}>{row.tipoIncidente || "Operación"}</span>
                      </div>
                      <p className="mt-2 line-clamp-2 text-base font-semibold leading-5 text-slate-950 dark:text-white">{row.descripcion || "Sin descripción"}</p>
                    </div>
                  ...
                  <div className="mt-3 flex items-center justify-between gap-3 text-xs font-semibold text-slate-500 dark:text-slate-400">
                    ...
                    <span className="inline-flex items-center gap-1 font-semibold text-emerald-700 dark:text-emerald-300">Ver detalle <ChevronRight className="h-4 w-4" /></span>
                  </div>
                </button>
              </GuidedTarget>
            ))}
      ...
      <div>
        <div className={cn("overflow-x-auto rounded-lg border border-slate-200 dark:border-slate-800", isStale && "opacity-70")}>
          <table className="w-full min-w-[1120px] table-fixed text-left text-sm">
            ...
            <thead className="border-b border-slate-200 bg-slate-50 text-[11px] font-semibold uppercase tracking-[.12em] text-slate-500 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-400">
                ...
                >
                  <td className="p-0">
                    <button type="button" onClick={() => onRowPress?.(row)} className="block w-full px-4 py-4 text-left focus:outline-none focus-visible:ring-2 focus-visible:ring-inset focus-visible:ring-emerald-500" aria-label={`Ver incidente ${row.id ?? ""}`}>
                      <div className="flex items-center gap-2"><span className="font-mono text-xs font-semibold text-emerald-700 dark:text-emerald-300">#{row.id ?? "—"}</span><span className="text-xs font-semibold text-slate-400">{row.fecha || "Sin fecha"}</span></div>
                      <p className="mt-1 line-clamp-2 font-bold leading-5 text-slate-900 dark:text-white">{row.descripcion || "Sin descripción"}</p>
                    </button>
                  </td>
                  <td className="px-4 py-4"><span className={`inline-flex rounded-full px-2.5 py-1 text-[10px] font-semibold uppercase tracking-wide ${operationTone(row)}`}>{row.tipoIncidente || "Operación"}</span></td>
                  ...
                  <td className="px-4 py-4"><div className="flex items-center justify-between gap-2"><StatusBadge status={row.estatus} label={row.estatus} tone={incidentStatusTone(row.estatus)} size="sm" dot /><button type="button" onClick={() => onRowPress?.(row)} className="grid h-11 w-11 place-items-center rounded-lg text-slate-400 transition hover:bg-white hover:text-emerald-700 hover:shadow-sm dark:hover:bg-slate-800" aria-label={`Abrir incidente ${row.id ?? ""}`}><ChevronRight className="h-4 w-4" /></button></div></td>
                </GuidedTarget>
              ))}
            </tbody>
          </table>
