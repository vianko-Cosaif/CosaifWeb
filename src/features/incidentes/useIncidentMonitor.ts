...
export function useIncidentMonitor({
  ...
  const lastIncidentIdsRef = useRef(new Set<number>());
  ...
  const runIncidentCheck = useCallback(async (): Promise<void> => {
    ...
    try {
      ...
      if (data?.success && Array.isArray(data.data)) {
        ...
        throw new Error("La respuesta de incidentes no tiene un formato válido");
      }
      ...
      // Los callbacks quedan fuera de un actualizador de estado: React puede
      // ejecutar esos actualizadores más de una vez en Strict Mode.
      const previousIds = lastIncidentIdsRef.current;
      const newIncidents = adaptedIncidents.filter((incident) => !previousIds.has(incident.id));
      lastIncidentIdsRef.current = new Set(adaptedIncidents.map((incident) => incident.id));
      if (newIncidents.length > 0) {
        setHasNewIncidents(true);
        newIncidents.forEach((incident) => onIncidentDetectedRef.current?.(incident));
      } else if (adaptedIncidents.length === 0) {
      ...
      setLastCheck(new Date(lastSuccessfulCheckRef.current));
    } catch (err: any) {
      if (mySeq === requestSeq.current && isMounted.current) {
        // Cambiar de ámbito, pausar o esconder la pestaña no es un fallo HTTP.
  ...
  }, [apiBase, empresaId, localidadId, adaptIncidente, handleFetchRequest]);
  ...
  // Focus, visibilidad y conexión realtime pueden llegar juntos. Comparten la
  // solicitud pendiente y no vuelven a descargar una respuesta recién obtenida.
  const checkIfStale = useCallback((maxAgeMs = 5_000): Promise<void> => {
    const lastSuccess = lastSuccessfulCheckRef.current;
    if (lastSuccess !== null && Date.now() - lastSuccess < maxAgeMs) return Promise.resolve();
    return checkIncidents();
  }, [checkIncidents]);
  ...
  useEffect(() => {
    lastSuccessfulCheckRef.current = null;
    lastIncidentIdsRef.current = new Set();
    setActiveIncidents([]);
    setHasNewIncidents(false);
    setLastCheck(null);
    setError(null);
  }, [apiBase, empresaId, localidadId]);

  // Un único dueño del polling y de la recuperación de foco/visibilidad.
  // El intervalo no se reinicia al renderizar una alerta o actualizar lastCheck.
  useEffect(() => {
    const cancelPending = () => {
      requestSeq.current += 1;
      requestControllerRef.current?.abort();
    ...
    if (!enabled || !isMonitoring) return cancelPending;
    ...
    const onVisibility = () => {
      if (document.visibilityState === "hidden") {
        // Se pueden recibir cambios realtime mientras no consultamos. Al volver,
        // renovar el estado aunque la última respuesta tenga pocos segundos.
        lastSuccessfulCheckRef.current = null;
        cancelPending();
      } else refreshVisible();
    };
    refreshVisible();
    const timerId = window.setInterval(refreshVisible, Math.max(1_000, intervalMs));
  ...
  }, [enabled, isMonitoring, intervalMs, checkIfStale]);
