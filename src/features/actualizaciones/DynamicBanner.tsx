...
import type { BannerLayer, DashboardBannerConfig } from "./DynamicBanner.types";
...
const resolveCanvasMetrics = (banner: DashboardBannerConfig["banner"]) => {
  const designWidth =
...
const resolveRootBackground = (banner: DashboardBannerConfig["banner"], apiBase: string) => {
...
const hasRenderableBackground = (banner: DashboardBannerConfig["banner"]) => {
  const background = banner?.background;
...
const resolveLayerChildren = (layer: BannerLayer): BannerLayer[] => {
  const list = layer.components ?? layer.children;
