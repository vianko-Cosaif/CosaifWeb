...
import QueueSegmentedFilter, { type QueueSegmentedFilterOption } from "../../cliente/components/QueueSegmentedFilter";
...
export default function RailQueueBoard({
  ...
  const realtimeStatus = useRealtimeBoardRefresh({
    ...
    onRefresh: ({ event }) => {
      ...
      if (event.type === "realtime.ready" && (loading || (updatedAt && Date.now() - updatedAt < 20_000))) return;
    ...
    <GuidedTarget id="dashboard-rounds-board" className={S.Layout.root}>
      ...
      <GuidedTarget id="dashboard-rounds-header" as="header" className={S.Layout.header}>
        <div className={S.Header.left}>
          <h1 className={S.Header.title}>{sharedLocality ? "Ronda general de la localidad" : "Rondas actuales"}</h1>
          <span className={S.Header.liveBadge}>{realtimeStatus === "connected" && polling ? <><span className={S.Header.liveDot} /> EN VIVO</> : polling ? "Actualización automática" : "Actualización manual"}</span>
        </div>
        <div className={S.Header.right}>
          ...
          <button type="button" onClick={() => load(true)} disabled={refreshing || loading} className={S.Header.btn()} title="Actualizar" aria-label="Actualizar rondas" aria-busy={refreshing}>
      ...
      <div className={S.Layout.main} aria-busy={loading || refreshing}>
        <p className="text-xs leading-5 text-[var(--app-text-muted)] lg:col-span-12">{sharedLocality ? "Actuales de todas las empresas de tu localidad: pendientes, en movimiento y detenidos." : "Rondas actuales de la localidad disponibles para tu perfil."} Las acciones y las mediciones siguen limitadas a tu empresa.</p>
        {error && <div role="alert" className="rounded-lg border border-amber-300 bg-amber-50 px-4 py-3 text-sm text-amber-900 dark:border-amber-800 dark:bg-amber-950/30 dark:text-amber-200 lg:col-span-12">{error} {items.length > 0 ? "Se conserva la última consulta de esta selección; puede haber cambiado." : "No se pudo consultar esta selección."}<button type="button" onClick={() => load(true)} className="ml-2 font-semibold underline">Reintentar</button></div>}
        <GuidedTarget
        ...
        >
          ...
          <GuidedTarget id="dashboard-status-tabs">
            <span className="px-2 text-xs font-semibold text-[var(--app-text-muted)]">Actuales de la localidad</span>
          </GuidedTarget>
        </GuidedTarget>
        {/* LEFT — Hero */}
