...
export const S = {
  ...
  Layout: {
    ...
    header: [
      "relative z-30 xl:sticky xl:top-0",
      "min-h-12 gap-2 px-4 py-2 md:px-5",
  ...
  /* ── HEADER ─────────────────────────────── */
  Header: {
    left: "flex min-w-0 flex-wrap items-center gap-2.5",
    title: "text-xl font-semibold tracking-tight text-[var(--app-text)]",
    ...
    btn: (active?: boolean) =>
      `h-11 w-11 shrink-0 inline-flex items-center justify-center rounded-md text-xs ${TR} ` +
      (active
        ? "bg-[var(--app-surface-muted)] text-[var(--app-text)]"
        : "text-[var(--app-text-soft)] hover:bg-[var(--app-surface-muted)] hover:text-[var(--app-text)]"),
    btnEdit: `min-h-11 inline-flex items-center gap-1.5 px-2 sm:px-3 rounded-md text-xs font-semibold ${TR} bg-[var(--app-accent)] text-[var(--app-accent-contrast)] hover:bg-[var(--app-accent-hover)] active:scale-[0.97]`,
  },
  ...
  Card: {
    ...
    statLabel: "text-xs font-medium text-[var(--app-text-muted)] mb-1 flex items-center gap-1",
    ...
    chipLabel: "text-xs font-medium text-[var(--app-text-muted)] mb-1",
    chipValue: "break-words text-sm font-semibold tabular-nums",
  ...
  Services: {
    ...
    label: "text-xs font-medium text-[var(--app-text-muted)] mb-2",
    pillWrap: "flex flex-wrap gap-1.5",
    pill: (on: boolean) =>
      `inline-flex items-center gap-1 px-2 py-0.5 rounded text-xs font-medium border ${TR} ${on
