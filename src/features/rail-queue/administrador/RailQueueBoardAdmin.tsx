...
import { AlertTriangle, ArrowRight, Check, Expand, Minimize, MapPin, RefreshCw, TrainFront, Volume2, VolumeX, WifiOff } from "lucide-react";
...
const TornoMeasuresViewerModal = dynamic(() => import("../../movimientos/torno/TornoMeasuresViewerModal"), { ssr: false });
...
export default function RailQueueBoardAdmin({ autoMs = 120_000, nextCount = 5 }: { autoMs?: number; nextCount?: number }) {
  ...
  useEffect(() => {
    ...
    void import("@/lib/firebase").then(({ syncFirebaseNotificationLocalidad }) => syncFirebaseNotificationLocalidad(activeLocId)).catch((error) => {
  ...
  const loadLocalidades = useCallback(async (signal?: AbortSignal) => {
    try {
      ...
      if (!signal?.aborted) setCatalogError("No se pudo cargar el catálogo de localidades. Intenta actualizar.");
    }
  }, []);
  ...
  async function load(showRefreshing = false, force = showRefreshing) {
    ...
    const requested = activeLocId > 0
      ? [localidades.find((loc) => loc.id === activeLocId) ?? { id: activeLocId, nombre: `Localidad #${activeLocId}` }]
      : localidades;
      ...
      try {
        const result = await loadAdminRounds({
          ...
          onLocalidadLoaded: (loc, list) => {
            ...
            setLoadedLocIds((previous) => previous.includes(loc.id) ? previous : [...previous, loc.id]);
            setUnavailableLocIds((previous) => previous.filter((id) => id !== loc.id));
          },
        });
        if (mySeq !== reqSeq.current || ac.signal.aborted) return;
        const unavailable = result.unavailableLocalidades;
        setUnavailableLocIds((previous) => [...previous.filter((id) => !requested.some((loc) => loc.id === id)), ...unavailable.map((loc) => loc.id)]);
        ...
        if (!firstLoad.current) {
          ...
          if (activeLocId > 0 && prev.length && nextIds[0] && nextIds[0] !== prev[0]) {
            pushToast(`Se movió la orden a ${codeFrom(rondaInfoMap([data[0]])[data[0].id], data[0].id)}`, "move");
          }
          if (created.length) pushToast(`Nueva(s): ${fmtList.format(created.map((item) => codeFrom(rondaInfoMap([item])[item.id], item.id)))}`, "new");
  ...
  useRealtimeBoardRefresh({
    ...
    onRefresh: ({ event }) => {
      ...
      if (event.type === "realtime.ready" && (inFlightRef.current || (lastOkAt && Date.now() - lastOkAt < 20_000))) return;
  ...
  useEffect(() => {
    ...
    inFlightRef.current = null;
    pendingReloadRef.current = false;
    firstLoad.current = true; prevIdsRef.current = []; setItems([]); setError(null); setLoadedLocIds([]); setUnavailableLocIds([]); setLastOkAt(null); setLoading(true);
    if (activeIsTorreon) {
      setLoading(false);
      setRefreshing(false);
      return;
  ...
  }, [activeIsTorreon, activeLocId, activeLocId === 0 ? localidadesReady : true, activeLocId === 0 ? localidades.map(l => l.id).join(",") : "single"]);
  ...
  const toggleFullscreen = async () => {
    ...
    try {
      ...
      pushToast("No se pudo cambiar la pantalla completa. Puedes seguir usando el tablero e intentarlo de nuevo.", "error");
    } finally {
    ...
    <div ref={boardRef} data-admin-dashboard="true" data-fullscreen={isFs} className={styles.board}>
      ...
      <header className={styles.header}>
        <div className="flex items-center gap-3">
          <span className="grid h-11 w-11 shrink-0 place-items-center rounded-xl bg-slate-900 text-emerald-300"><TrainFront size={22} aria-hidden /></span>
          <div className="min-w-0">
            <p className="text-[11px] font-semibold uppercase tracking-[0.18em] text-[var(--app-text-muted)]">Administración ferroviaria</p>
            <h1 className="mt-1 text-xl font-semibold tracking-tight sm:text-2xl">Control de operaciones</h1>
          </div>
        </div>
        <p className="mt-3 max-w-2xl text-sm leading-6 text-[var(--app-text-muted)]">Rondas, prioridades y órdenes por localidad. Selecciona un patio para consultar su operación.</p>
      </header>
      {/* TOOLBAR */}
            ...
            <div className="flex min-w-0 max-w-full flex-wrap items-center gap-3 rounded-2xl border border-slate-200/70 bg-[var(--app-surface)] px-3 py-2 shadow-sm dark:border-slate-800/70">
              ...
              <div className="relative min-w-0 flex-1">
                ...
                >
                  ...
                  {activeLocId > 0 && !activeLoc && <option value={activeLocId}>Localidad #{activeLocId}</option>}
                  {localidades.map((l) => (
            ...
            <div className="flex flex-wrap items-center gap-2">
              {activeIsTorreon ? (
                ...
                <>
                  ...
                  <Btn onClick={toggleSound} active={soundOn} labelOn="Sonido" labelOff="Silencio" iconOn={<Volume2 size={15} />} iconOff={<VolumeX size={15} />} />
                  <Btn onClick={() => setPolling((p) => !p)} active={polling} labelOn="Auto" labelOff="Auto" iconOn={<Check size={15} />} iconOff={<RefreshCw size={15} />} />
                  <Btn onClick={() => { if (!localidadesReady) void loadLocalidades(); else void load(true); }} disabled={refreshing || (loading && !catalogError)} labelOn={refreshing ? "Actualizando…" : "Actualizar"} iconOn={<RefreshCw size={15} className={refreshing ? "animate-spin" : ""} />} />
                </>
              )}
              <Btn onClick={toggleFullscreen} active={isFs} disabled={fullscreenPending} labelOn={isFs ? "Salir de pantalla completa" : "Pantalla completa"} iconOn={isFs ? <Minimize size={15} /> : <Expand size={15} />} />
            </div>
          </div>
        </div>
      </div>
      ...
      <section className={styles.content} aria-busy={loading || refreshing}>
        {!online && <div role="status" className="mb-4 flex items-center gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900"><WifiOff size={17} /> Sin conexión. La información puede haber cambiado.</div>}
        {(error || catalogError) && !activeIsTorreon && <div role="alert" className="mb-4 flex items-start gap-2 rounded-xl border border-amber-200 bg-amber-50 p-3 text-sm text-amber-900 dark:border-amber-900 dark:bg-amber-950/30 dark:text-amber-200"><AlertTriangle size={18} className="mt-0.5 shrink-0" /><span>{error || catalogError}</span></div>}
        {!activeIsTorreon && <QueueSummary summary={summary} pending={loading} partial={unavailableLocIds.length > 0 || (activeLocId === 0 && Boolean(catalogError))} />}
        {!activeIsTorreon && activeLocId === 0 && loading && localidades.length > 0 && <p role="status" className="mb-4 text-xs text-[var(--app-text-muted)]">Consultando localidades: {loadedLocIds.length} de {localidades.length}. Puedes abrir cualquier patio.</p>}
      ...
      {measuresModal.open && (measuresModal.loading || measuresModal.error) ? (
        <div className="fixed inset-0 z-[90] flex items-center justify-center bg-black/35 p-4">
          <div className="w-full max-w-md rounded-xl border border-slate-200 bg-[var(--app-surface)] p-4 shadow-xl dark:border-slate-700">
            {measuresModal.loading ? (
              <p className="text-sm text-[var(--app-text-muted)]">Cargando medidas de torno...</p>
            ) : (
...
function LastUpdated({ live, timestamp }: { live: boolean; timestamp: number | null }) {
  ...
  return <Badge live={live} label={timestamp ? `Actualizado hace ${timeAgo(timestamp)}` : "Esperando actualización"} />;
}

function QueueSummary({ summary, pending, partial }: { summary: ReturnType<typeof summarizeAdminQueue>; pending: boolean; partial: boolean }) {
  const metrics = [
    { label: "Órdenes en ronda", value: summary.total, tone: "text-[var(--app-text)]" },
    { label: "Localidades con órdenes", value: summary.activeLocalidades, tone: "text-[var(--app-text)]" },
    { label: "Prioridad alta", value: summary.highPriority, tone: "text-amber-700 dark:text-amber-300" },
    { label: "Movimientos detenidos", value: summary.stopped, tone: "text-rose-700 dark:text-rose-300" },
  ];
  return <div className="mb-5">
    <div className="grid grid-cols-2 gap-3 xl:grid-cols-4">{metrics.map((metric) => <div key={metric.label} className="min-w-0 rounded-xl border border-slate-200 bg-[var(--app-surface)] px-4 py-3 dark:border-slate-800">
      <p className="text-xs leading-5 text-[var(--app-text-muted)]">{metric.label}</p>
      <p className={`mt-2 text-2xl font-semibold tabular-nums ${metric.tone}`}>{pending || partial ? "—" : metric.value}</p>
    </div>)}</div>
    <p className="mt-2 text-[11px] text-[var(--app-text-muted)]">{partial ? "Totales no disponibles: hay localidades sin actualizar." : pending ? "Los totales estarán disponibles al completar la consulta." : "Resumen de las rondas devueltas para la selección actual."}</p>
  </div>;
}
...
function Btn({ onClick, active, disabled, labelOn, labelOff, iconOn, iconOff }:
  { onClick: () => void; active?: boolean; disabled?: boolean; labelOn: string; labelOff?: string; iconOn: ReactNode; iconOff?: ReactNode }) {
  return (
    <button
      type="button"
      onClick={onClick}
    ...
    >
      <span className="text-sm" aria-hidden>{active ? iconOn : (iconOff ?? iconOn)}</span>
...
function SingleLocalidadBoard(props: {
  ...
  return (
    <div className="grid gap-4 md:gap-6 lg:gap-8 lg:grid-cols-3">
      {/* Izquierda: actual */}
      <div className={`${styles.currentCard} lg:col-span-2 rounded-2xl p-4 sm:p-6 shadow-sm`}>
        <div className="mb-4 flex flex-col sm:flex-row sm:items-center sm:justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold tracking-tight text-[var(--app-text)]">
              Operación del patio
            </h2>
            <div className="mt-1 text-xs tracking-widest font-medium text-[var(--app-text-muted)] uppercase">
              Orden Actual
            </div>
          </div>
          <button
            onClick={() => load(true)}
            className="text-xs rounded-full px-4 py-2 border bg-[var(--app-surface)] hover:bg-slate-50 transition-all duration-200 dark:hover:bg-slate-700 dark:border-slate-600 hover:scale-105 active:scale-95 flex items-center gap-2"
            disabled={refreshing}
        ...
        <div
          ...
          className="rounded-xl bg-[var(--app-surface)] p-4 sm:p-6 border shadow-sm border-slate-200 dark:border-slate-700 min-h-[200px]"
        >
          {loading && !current ? (
            <SkeletonCurrent />
          ) : current ? (
            <>
              <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4 mb-6">
                <div className="flex items-center gap-3 sm:gap-4">
                  <div className="grid h-12 w-12 sm:h-14 sm:w-14 place-items-center rounded-full bg-gradient-to-br from-sky-100 to-emerald-100 border border-slate-200 dark:from-slate-800 dark:to-slate-700 dark:border-slate-600">
                    <TrainFront className="h-6 w-6 text-emerald-700 dark:text-emerald-300" />
                  </div>
                  <div>
                    <div className="text-xs uppercase tracking-widest text-[var(--app-text-muted)]">Locomotora</div>
                    <div className="text-lg font-bold text-[var(--app-text)]">{locoText}</div>
                    <div className="mt-1 text-sm text-[var(--app-text-muted)]">{curInfo?.empresa?.nombre ?? "—"}</div>
                  </div>
                </div>
                <div className="text-right">
                  <div className="text-xs text-[var(--app-text-muted)]">Código</div>
                  <div className="break-all text-2xl font-bold tracking-tight text-emerald-700 dark:text-emerald-300 sm:text-3xl">
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-3 mb-4">
                ...
                <div className="rounded-xl border border-slate-200 bg-[var(--app-surface-subtle)] p-3 dark:border-slate-700">
                  <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-[var(--app-text-muted)] mb-2">
              ...
              <div className="mb-6 rounded-xl border border-slate-200 bg-[var(--app-surface)] p-4 text-slate-800 shadow-sm dark:border-slate-700 dark:text-slate-100">
                <div className="text-[11px] uppercase tracking-[0.25em] text-[var(--app-text-muted)] mb-2">
      ...
      {/* Derecha: próximas */}
      <aside className={`${styles.nextOrders} rounded-2xl shadow-sm p-4 sm:p-6`}>
        <div className="mb-4 flex items-center justify-between sticky top-0 bg-[var(--app-surface)] pb-2">
          <h3 className="font-semibold text-base">Siguientes órdenes</h3>
          <span className="rounded-full border border-slate-200 bg-[var(--app-surface-muted)] px-3 py-1 text-xs text-slate-700 dark:border-slate-700 dark:text-slate-300">
            ...
            {loading && next.length === 0 ? (
              ...
              next.map((n) => {
                ...
                return (
                  <div
                    key={n.id}
                    className="rounded-lg border border-slate-200 bg-[var(--app-surface-subtle)] p-4 transition-all duration-200 hover:bg-white hover:shadow-md hover:border-slate-300 dark:border-slate-700 dark:hover:bg-slate-800"
                  >
...
function KV({ title, value, prefix }: { title: string; value: string; prefix: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-[var(--app-surface-subtle)] p-3 dark:border-slate-700">
      <div className="flex items-center gap-2 text-xs uppercase tracking-wider text-[var(--app-text-muted)] mb-1">
        <span>{prefix}</span> {title}
      </div>
      <div className="text-sm font-semibold text-[var(--app-text)] truncate">{value}</div>
    </div>
  );
}
  ...
  return (
    <>
      <div className="flex items-center gap-3 mb-3">
        <div className="grid h-10 w-10 place-items-center rounded-full border border-slate-200 bg-[var(--app-surface)] dark:border-slate-700">
          <TrainFront className="h-5 w-5 text-emerald-700 dark:text-emerald-300" />
        </div>
        <div className="min-w-0 flex-1">
          <div className="text-xs text-[var(--app-text-muted)]">Código</div>
          <div className="truncate font-bold tracking-wide text-[var(--app-text)]">{codeFrom(inf, n.id)}</div>
          <div className="truncate text-xs text-[var(--app-text-muted)]">{inf?.empresa?.nombre ?? "—"}</div>
        </div>
        <div className="whitespace-nowrap text-xs text-[var(--app-text-muted)]">Ronda #{n.rondaNumero}</div>
      </div>
      ...
      <div className="flex flex-wrap items-center justify-between gap-2">
        <div className="text-xs text-[var(--app-text-muted)]">Loco: <span className="font-medium text-[var(--app-text)]">{loco}</span></div>
      ...
      <div className="mt-3 grid grid-cols-1 gap-2">
        <div className="rounded-lg border border-slate-200 bg-[var(--app-surface)] p-2 text-xs text-[var(--app-text-muted)] dark:border-slate-700 dark:text-slate-200">
          <span className="font-semibold text-[var(--app-text-muted)]">Instrucciones:</span>{" "}
          {instrText}
        </div>
...
function Cell({ title, value, solid }: { title: string; value: string; solid?: boolean }) {
  return (
    <div className={`rounded-lg border ${solid ? "border-slate-300 bg-[var(--app-surface)] dark:border-slate-600" : "border-slate-200 bg-[var(--app-surface)] dark:border-slate-700"} p-2`}>
      <div className="text-xs text-[var(--app-text-muted)]">{title}</div>
...
const AllLocalidadesGrid = memo(function AllLocalidadesGrid({
  localidades, itemsByLoc, loading, loadedLocIds, unavailableLocIds, onSelect,
}: {
  ...
  for (const [id, list] of itemsByLoc ?? []) {
    if (!metadata.has(id)) metadata.set(id, { id, nombre: list[0]?.localidad?.nombre ?? `Localidad #${id}` });
  }
  if (!metadata.size) return loading
    ? <div className="grid gap-4 md:grid-cols-2 xl:grid-cols-3">{Array.from({ length: 3 }).map((_, index) => <SkeletonNext key={index} />)}</div>
    : <EmptyState />;
  const loaded = new Set(loadedLocIds);
  const unavailable = new Set(unavailableLocIds);
  return <div className={`grid items-start gap-4 ${metadata.size === 1 ? "grid-cols-1" : metadata.size === 2 ? "md:grid-cols-2" : "md:grid-cols-2 xl:grid-cols-3"}`}>
    {Array.from(metadata.values()).map((localidad) => {
      ...
      return <article key={localidad.id} className="min-w-0 overflow-hidden rounded-xl border border-slate-200 bg-[var(--app-surface)] dark:border-slate-800" style={{ contentVisibility: "auto", containIntrinsicSize: "auto 300px" }}>
        <div className="flex items-start justify-between gap-3 border-b border-slate-100 p-4 dark:border-slate-800">
          <div className="min-w-0"><h2 className="break-words font-semibold">{localidad.nombre}</h2><p className="mt-1 text-xs text-[var(--app-text-muted)]">{localidad.estado || "Operación ferroviaria"}</p></div>
          <span className={`shrink-0 rounded-md px-2 py-1 text-[11px] font-medium ${failed ? "bg-amber-50 text-amber-800 dark:bg-amber-950/30 dark:text-amber-200" : "bg-[var(--app-surface-muted)] text-[var(--app-text-muted)] dark:text-slate-300"}`}>{failed ? "Sin actualizar" : pending ? "Consultando" : `${list.length} ${list.length === 1 ? "orden" : "órdenes"}`}</span>
        </div>
        <div className="space-y-3 p-4">
          {pending && !current ? <div className="animate-pulse space-y-3 py-2"><div className="h-4 w-2/3 rounded bg-[var(--app-surface-muted)]" /><div className="h-12 rounded bg-[var(--app-surface-muted)]" /></div>
            : current ? <>
              <div className="flex items-start gap-2"><TrainFront size={18} className="mt-0.5 shrink-0 text-emerald-700 dark:text-emerald-300" /><div className="min-w-0"><p className="break-words text-base font-semibold">Locomotora {fmtLoco(movement?.locomotiveNumber ?? movement?.locomotora)}</p><p className="mt-1 break-words text-xs text-[var(--app-text-muted)]">{currentInfo?.empresa?.nombre ?? "—"} · Orden {codeFrom(currentInfo, current.id)}</p></div></div>
              <div className="grid grid-cols-[1fr_auto_1fr] items-center gap-2 rounded-lg bg-[var(--app-surface-subtle)] p-3"><div className="min-w-0"><p className="text-[10px] uppercase text-[var(--app-text-muted)]">Origen</p><p className="mt-1 break-words text-xs font-medium">{movement?.viaOrigen?.nombre || "—"}</p></div><ArrowRight size={15} className="text-[var(--app-text-muted)]" /><div className="min-w-0"><p className="text-[10px] uppercase text-[var(--app-text-muted)]">Destino</p><p className="mt-1 break-words text-xs font-medium">{movement?.viaDestino?.nombre || "—"}</p></div></div>
              <div className="flex flex-wrap gap-2 text-[11px]"><span className="rounded border border-slate-200 px-2 py-1 dark:border-slate-700">Ronda {current.rondaNumero} · Posición {current.orden}</span><span className={`rounded px-2 py-1 ${movement?.estado === "DETENIDO" ? "bg-rose-50 text-rose-700 dark:bg-rose-950/30 dark:text-rose-300" : "bg-[var(--app-surface-muted)] text-[var(--app-text-muted)] dark:text-slate-300"}`}>{movement?.estado || "Sin estado"}</span>{movement?.prioridad === "ALTA" && <span className="rounded bg-amber-50 px-2 py-1 text-amber-800 dark:bg-amber-950/30 dark:text-amber-200">Prioridad alta</span>}</div>
              <details className="text-xs text-[var(--app-text-muted)]"><summary className="cursor-pointer py-1 font-medium">Instrucciones y fechas</summary><p className="mt-2 whitespace-pre-wrap break-words leading-5">{movement?.instrucciones?.trim() || "Sin instrucciones."}</p><p className="mt-2">Solicitud: {formatDateTimeMX(movement?.fechaSolicitud)}</p><p>Inicio: {formatDateTimeMX(movement?.fechaInicio)}</p><p>Fin: {formatDateTimeMX(movement?.fechaFin)}</p></details>
            </> : <p className="py-5 text-sm leading-6 text-[var(--app-text-muted)]">{failed ? "No se pudo consultar este patio." : special ? "Consulta naturales y arrastres en la operación de Torreón." : "Sin órdenes en ronda."}</p>}
          <button type="button" onClick={() => onSelect(localidad.id)} className="flex w-full items-center justify-between gap-2 rounded-lg border border-slate-200 px-3 py-2.5 text-xs font-semibold text-slate-700 transition-colors hover:border-emerald-300 hover:bg-emerald-50 focus-visible:outline focus-visible:outline-2 focus-visible:outline-emerald-600 dark:border-slate-700 dark:text-slate-200 dark:hover:bg-emerald-950/30">Ver operación de {localidad.nombre}<ArrowRight size={14} className="shrink-0" /></button>
        </div>
      </article>;
    })}
  </div>;
...
function InfoBadge({ label, value, icon }: { label: string; value: string; icon: string }) {
  return (
    <div className="rounded-lg border border-slate-200 bg-[var(--app-surface)] p-2 text-center dark:border-slate-700">
      <div className="flex items-center justify-center gap-1 text-[10px] uppercase tracking-wider text-[var(--app-text-muted)]">
        {icon} {label}
      </div>
      <div className="mt-1 text-sm font-semibold text-[var(--app-text)] truncate">{value}</div>
    </div>
  );
}
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-3 py-1 text-xs font-medium border transition-all duration-200 ${
      ok ? "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200"
         : "border-slate-200 bg-[var(--app-surface-muted)] text-[var(--app-text-muted)] dark:border-slate-700 dark:text-slate-300"}`}>
      {icon} {children}
    </span>
  );
}
  ...
  return (
    <span className={`inline-flex items-center gap-1 rounded-full px-2 py-1 text-[10px] border ${
      active ? "border-emerald-200 bg-emerald-50 text-emerald-700 dark:border-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-200"
             : "border-slate-200 bg-[var(--app-surface-muted)] text-[var(--app-text-muted)] dark:border-slate-700 dark:text-slate-400"}`}>
      {icon} {text}
    </span>
  );
}
    ...
    <div className="animate-pulse space-y-4">
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="flex items-center gap-4">
          <div className="h-12 w-12 sm:h-14 sm:w-14 rounded-full bg-slate-200" />
        ...
        <div className="h-8 sm:h-12 w-full sm:w-32 rounded bg-slate-200" />
      </div>
      <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
        {Array.from({ length: 3 }).map((_, i) => <div key={i} className="h-16 rounded-xl bg-slate-200" />)}
      </div>
      <div className="grid grid-cols-2 sm:grid-cols-4 gap-2">
        {Array.from({ length: 4 }).map((_, i) => <div key={i} className="h-12 rounded-lg bg-slate-200" />)}
      </div>
...
function DateBox({ label, value }: { label: string; value: string }) {
  return (
    <div className="rounded-xl border border-slate-200 bg-[var(--app-surface)] p-3 text-slate-700 shadow-sm dark:border-slate-700 dark:text-slate-200">
      <div className="text-[10px] uppercase tracking-[0.25em] text-[var(--app-text-muted)]">{label}</div>
...
function EmptyState() {
  return (
    <div className="py-12 text-center">
      ...
      <div className="text-sm text-[var(--app-text-muted)] mt-2">No hay órdenes en la cola actualmente</div>
    </div>
  );
}
