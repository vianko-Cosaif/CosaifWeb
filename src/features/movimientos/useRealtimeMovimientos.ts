...
type StreamState = {
  ...
  configController: AbortController | null;
...
async function connectStream(url: string, token: number) {
  ...
  try {
    ...
    markActivity();
    // The handshake is complete. A long-lived SSE body must not prevent
    // lifecycle handlers from recovering a connection that later goes stale.
...
async function resolveWebSocketUrl(configUrl: string): Promise<string> {
  ...
  streamState.configController = controller;
  ...
  try {
    ...
    // fetch resolves at response headers; the body is still part of the handshake.
    window.clearTimeout(timeoutId);
    if (streamState.configController === controller) streamState.configController = null;
  }
}
...
async function connectWebSocket(configUrl: string, token: number): Promise<void> {
  ...
  return new Promise((resolve, reject) => {
    ...
    ws.onmessage = (message) => {
      if (streamState.webSocket !== ws || streamState.connectionToken !== token) return;
...
function closeActiveConnection({ suppressWsReconnect = true } = {}) {
  streamState.configController?.abort();
  streamState.configController = null;
  streamState.abortController?.abort();
  streamState.abortController = null;
  clearWebSocketHealthTimer();
...
function bindLifecycleListeners() {
  ...
  window.addEventListener("cosaif:session-ended", stopRealtime);
...
export function useRealtimeMovimientos({
  ...
  // Server rendering has no browser channel. Read the shared live snapshot in
  // the subscription effect so the first client render matches the server.
