...
export default function CrearMovimiento() {
  ...
  const {
    ...
    online,
    pendingItems, syncing, retryPending, discardPending, savedAt, saveError,
    flushOutbox,
    submit,
    validate1,
    validate2,
  ...
  return (
      <div className={Movimiento.clsx(
        "create-movement-screen min-h-screen overflow-x-hidden p-2 text-slate-900 transition-colors duration-200 dark:text-white min-[380px]:p-3 sm:p-4 md:p-6 lg:p-8",
        guidedMode
      ...
      <style jsx global>{`
        @media (max-width: 640px) {
          .create-movement-screen select, .create-movement-screen select option { font-size: 16px !important; line-height: 1.45 !important; }
          .create-movement-screen select { min-height: 44px !important; }
        }
      ...
      >
        ...
        <div className={Movimiento.clsx("flex min-w-0 flex-wrap items-center gap-2", guidedMode ? "mb-2" : "mb-4")}>
          ...
          />
          {!sandboxMode && <OutboxPanel items={pendingItems} syncing={syncing} onRetry={retryPending} onDiscard={discardPending} onSync={flushOutbox} />}
          {!sandboxMode && <span role="status" className="text-xs text-slate-500">{saveError || (savedAt ? `Borrador guardado a las ${new Date(savedAt).toLocaleTimeString("es-MX", { hour: "2-digit", minute: "2-digit" })}` : "Borrador en este dispositivo")}</span>}
