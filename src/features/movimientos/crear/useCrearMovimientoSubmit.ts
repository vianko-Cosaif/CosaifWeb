...
import type { EnqueueOptions } from "../offline/types";
...
export function useCrearMovimientoSubmit(args: {
  ...
  pushOutbox: (payload: unknown, endpoint?: string, options?: EnqueueOptions) => Promise<void>;
  ...
  const requestRef = useRef<{ signature: string; id: string; queued: boolean } | null>(null);
  ...
  const submit = useCallback(async () => {
    ...
    try {
      ...
      const signature = JSON.stringify({ owner: currentStorageScope(), payload });
      if (requestRef.current?.signature !== signature) requestRef.current = { signature, id: crypto.randomUUID(), queued: false };
      if (requestRef.current.queued) { alert("Esta solicitud ya está en la bandeja de pendientes. Revísala antes de enviarla de nuevo."); return; }
      requestIdForOffline = requestRef.current.id;
      if ("clientRequestId" in requestPayload) requestPayload.clientRequestId = requestIdForOffline;
      ...
      if (typeof navigator !== "undefined" && navigator.onLine === false) {
        if (disabledRef.current) return;
        await pushOutbox(requestPayload, createEndpoint, { requestId: requestIdForOffline });
        requestRef.current.queued = true;
        setSending(false);
        return;
      }
      ...
      const res = await Movimiento.fetchWithTimeout(createEndpoint, {
        ...
        headers: {
          ...
          "Idempotency-Key": requestIdForOffline,
          ...(currentStorageScope() ? { "X-Cosaif-Outbox-Owner": currentStorageScope()! } : {}),
      ...
      if (!res.ok) {
        if (res.status >= 500 || res.status === 408) {
          await pushOutbox(requestPayload, createEndpoint, { requestId: requestIdForOffline, uncertain: true });
          if (requestRef.current) requestRef.current.queued = true;
          return;
        }
        try {
      ...
      if (isAbort || isTypeErr) {
        if (!disabledRef.current && payloadForOffline !== null) {
          try {
            await pushOutbox(payloadForOffline, endpointForOffline, { requestId: requestIdForOffline, uncertain: true });
            if (requestRef.current) requestRef.current.queued = true;
          } catch (storageError) { alert(getErrorMessage(storageError)); }
        }
        return;
      }
