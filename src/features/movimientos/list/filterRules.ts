...
export { movementDateBoundary } from "@/lib/dateBoundary";
...
const positiveId = (value: unknown) =>
  Number.isSafeInteger(Number(value)) && Number(value) > 0 ? Number(value) : undefined;
...
export function movementFilterPolicy(scope: MovementFilterScope, ambito: Ambito) {
  ...
  if (scope.bloquearLocalidad) {
    ...
    policy.forcedLocalidadId =
      scope.authorization?.scope.mode === "GLOBAL"
        ? positiveId(scope.localidadId)
        : (policy.forcedLocalidadId ??
          positiveId(scope.authorization?.scope.localidadId) ??
          positiveId(scope.localidadId));
  }
  return policy;
}
...
export function normalizeMovementFilters(
  input: Partial<FiltrosMovimientos>,
  ambito: Ambito,
  scope: MovementFilterScope = {},
): FiltrosMovimientos {
  const policy = movementFilterPolicy(scope, ambito);
  const allowedStates: readonly string[] =
    (scope.authorization?.role ?? scope.rol) === "CLIENTE" && ambito === "actuales"
      ? CURRENT_QUEUE_STATES
      : MOVEMENT_STATES[ambito];
  const estado = [
    ...new Set(
      text(input.estado)
        .split(",")
  ...
  return {
    ...
    fechaCampo: ["solicitud", "inicio", "fin", "creacion"].includes(String(input.fechaCampo))
      ? input.fechaCampo
      : "solicitud",
    pagina: Math.min(10_000, Math.max(1, Math.trunc(Number(input.pagina) || 1))),
    tamPagina: [10, 20, 25, 50].includes(Number(input.tamPagina))
      ? Number(input.tamPagina)
      : Number(input.tamPagina) > 50
        ? 50
        : 25,
...
export function movementFilterError(filters: FiltrosMovimientos) {
  ...
  if ((filters.desde && !from) || (filters.hasta && !to))
    return "Selecciona fechas válidas para consultar movimientos.";
  if (from && to && from > to) return "La fecha desde no puede ser posterior a la fecha hasta.";
  if (
    [filters.locomotiveNumber, filters.locomotivePrefix].some(
      (value) => value && !/^\d+$/.test(value),
    )
  )
    return "El número de locomotora debe contener solo dígitos.";
  return null;
}
...
export function movementSearchParams(filters: FiltrosMovimientos, ambito: Ambito) {
  const params = new URLSearchParams({
    ...
    pageSize: String(filters.tamPagina),
    sortBy: filters.campoOrden,
    sortDir: filters.direccionOrden,
    ambito,
  });
