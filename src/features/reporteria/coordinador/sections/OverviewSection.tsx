...
export default function OverviewSection({
  ...
  const cards = [
    ...
    hasNumber(kpis.totalConFin)
      ...
      : null,
    hasNumber(kpis.totalSinFin)
      ? { key: "sinFin", label: "Sin fin", value: fmtMaybeInt(kpis.totalSinFin), accent: "rose" }
      : null,
    ...
    hasNumber(kpis.movimientosConIncidente)
      ...
      : null,
    hasNumber(kpis.cancelados)
      ? { key: "cancelados", label: "Cancelados", value: fmtMaybeInt(kpis.cancelados), accent: "amber" }
      : null,
    hasNumber(kpis.canceladosConIncidente)
      ? {
          key: "canceladosInc",
          label: "Cancelados con incidente",
          value: fmtMaybeInt(kpis.canceladosConIncidente),
          accent: "rose",
        }
      : null,
  ].filter(Boolean) as Array<{ key: string; label: string; value: string; accent: React.ComponentProps<typeof KpiCard>["accent"] }>;

  return (
    <div className="space-y-6">
      <section className="grid grid-cols-12 gap-4">
        {cards.map((c) => (
          <div key={c.key} className="col-span-12 md:col-span-6 xl:col-span-4">
      ...
      {hasArray(movimientosHora) ? (
        <section>
          <ChartCard title="Movimientos por hora" subtitle="Distribución diaria por hora" accent="sky">
            <div className="mb-4 flex flex-wrap gap-2 text-[11px] uppercase tracking-[0.18em] text-[var(--app-text-muted)]">
              <span className="rounded-full border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-1 text-[var(--app-text-muted)]">
