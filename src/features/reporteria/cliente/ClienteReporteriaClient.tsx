...
function readSessionEmpresa() {
  ...
  try {
    ...
    const storedId = Number(user?.empresaId ?? user?.empresa?.id ?? NaN);
    const id = getEmpresaIdClient() ?? (Number.isFinite(storedId) && storedId > 0 ? storedId : null);
    return {
      id,
      nombre: id === storedId ? String(user?.empresa?.nombre || "") : "",
    };
  } catch {
...
export default function ClienteReporteriaClient() {
  ...
  const [cronologiaPagination, setCronologiaPagination] = useState({ filterKey: "", page: 1 });
  ...
  const [reportSnapshot, setReportSnapshot] = useState<{ queryKey: string; report: AnyRecord; fetchedAt: Date } | null>(null);
  ...
  useEffect(() => {
    const nextEmpresa = readSessionEmpresa();
    const nextLocalidad = getLocIdClient();
    const role = getRoleClient();
    const requiresLocalidad = role === "CLIENTE" || role === "ARRASTRE_TORREON";
    setEmpresa(nextEmpresa);
    setLocalidadId(nextLocalidad);
    setScopeError(!nextEmpresa.id
      ? "No hay una empresa asignada a la sesión para consultar reportes."
      : requiresLocalidad && !nextLocalidad
        ? "No hay una localidad asignada a la sesión para consultar reportes."
        : null);
    setScopeReady(true);
  }, []);
  ...
  const filterKey = JSON.stringify([activeReport, anchorFecha, periodo, empresa.id, localidadId, useLocalidad]);
  const cronologiaPage = cronologiaPagination.filterKey === filterKey ? cronologiaPagination.page : 1;
  const setCronologiaPage = (page: number) => setCronologiaPagination({ filterKey, page });

  useEffect(() => {
    setCronologiaPagination(current => current.filterKey === filterKey ? current : { filterKey, page: 1 });
  }, [filterKey]);
  ...
  const queryKey = `${API_ROOT}/${activeDefinition.path}?${query.toString()}`;
  const report = reportSnapshot?.queryKey === queryKey ? reportSnapshot.report : null;
  const fetchedAt = reportSnapshot?.queryKey === queryKey ? reportSnapshot.fetchedAt : null;

  const fetchReport = useCallback(async () => {
    if (!scopeReady || scopeError) return;
    ...
    try {
      ...
      setReportSnapshot({ queryKey, report: (json?.reporte ?? json) as AnyRecord, fetchedAt: new Date() });
    } catch (e: unknown) {
      if (controller.signal.aborted || requestRef.current !== controller) return;
  ...
  }, [queryKey, scopeReady, scopeError]);
  ...
  const exportPdf = useCallback(async () => {
    if (pdfBusy || !report || !scopeReady || scopeError) return;
    setPdfBusy(true);
    setError(null);
    try {
  ...
  }, [activeDefinition.path, anchorFecha, pdfBusy, periodo, query, report, scopeReady, scopeError]);

  return (
    <div className={`${theme.root} w-full min-w-0 max-w-full space-y-4 sm:space-y-5 2xl:mx-auto 2xl:max-w-screen-2xl`}>
      <header className="flex flex-col gap-4 border-b border-slate-200/80 pb-4 dark:border-slate-800 xl:flex-row xl:items-end xl:justify-between">
        <div className="min-w-0 space-y-2">
          ...
          <div className="flex flex-wrap items-center gap-3">
            <h1 className="min-w-0 break-words text-xl font-semibold tracking-tight text-[var(--app-text)] sm:text-2xl">
        ...
        <div className="flex flex-col gap-2 sm:items-end">
          <div className="grid grid-cols-2 gap-2 sm:flex sm:flex-wrap sm:justify-end">
            <button
              type="button"
              onClick={fetchReport}
              disabled={loading || !scopeReady || Boolean(scopeError)}
              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg border border-slate-200 bg-white px-3 py-2 text-sm font-semibold text-slate-700 shadow-sm transition hover:border-slate-300 disabled:opacity-60 dark:border-slate-800 dark:bg-slate-950 dark:text-slate-200 sm:w-auto sm:px-4"
            >
            ...
            <button
              ...
              className="inline-flex min-h-11 w-full items-center justify-center gap-2 rounded-lg bg-slate-950 px-3 py-2 text-sm font-semibold text-white shadow-sm transition hover:bg-slate-800 disabled:opacity-60 dark:bg-white dark:text-slate-950 sm:w-auto sm:px-4"
            >
      ...
      {scopeError || error ? (
        <div role="alert" className="rounded-lg border border-rose-200 bg-rose-50 px-4 py-3 text-sm font-medium text-rose-700 dark:border-rose-900/60 dark:bg-rose-950/40 dark:text-rose-200">
          {scopeError || error}
        </div>
      ) : null}

      <section className="min-h-[480px]">
        {(!scopeReady || loading) && !report ? <LoadingState /> : null}
        {scopeReady && !loading && !report && !error && !scopeError ? <EmptyState /> : null}
        {report ? (
          <ReportContent
