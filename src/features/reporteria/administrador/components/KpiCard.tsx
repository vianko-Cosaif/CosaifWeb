...
export default function KpiCard({
  ...
  return (
    <div className="relative min-w-0 overflow-hidden rounded-2xl border border-[var(--stroke)] bg-[var(--panel)] p-4 shadow-[var(--shadow)]">
      <div className={`absolute inset-x-0 top-0 h-1.5 bg-gradient-to-r ${accentMap[accent]}`} />
      <div className="text-xs font-semibold uppercase tracking-[0.06em] text-[var(--muted)]">
        {title}
      </div>
      <div className="mt-2 break-words text-2xl font-semibold text-[var(--text)]">{value}</div>
