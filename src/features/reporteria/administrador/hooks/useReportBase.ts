...
export function useReportBase<TReport extends Reporte = Reporte>({
  endpoint,
  pdfEndpoint,
  ...
  const setEmpresaId = useCallback((value: string) => {
    setEmpresaIdState(value);
    if (!value || !localidadId || !localidades.some(item => item.empresaId != null)) return;
    const belongsToCompany = localidades.some(item => String(item.id) === localidadId && String(item.empresaId ?? "") === value);
    if (!belongsToCompany) setLocalidadId("");
  ...
  const [snapshot, setSnapshot] = useState<{ key: string; report: TReport | null; fetchedAt: Date } | null>(null);
  ...
  const queryKey = `${endpoint}|${periodoBack}|${anchorFecha}|${empresaId}|${localidadId}`;
  ...
  const fetchReport = useCallback(async (force = true) => {
    ...
    try {
      ...
      const json = await cachedFetchJson<unknown>(`${endpoint}?${qs}`, { credentials: "include", signal: controller.signal }, { force, ttlMs: 20_000 });
      if (controller.signal.aborted) return;
      const rep = ((json && typeof json === "object" && "reporte" in json ? json.reporte : json)) as TReport;
  ...
  }, [validate, empresaId, localidadId, endpoint, queryKey]);
  ...
  const ejecucionBuckets = useMemo(() => report?.ejecucionBuckets ?? [], [report?.ejecucionBuckets]);
