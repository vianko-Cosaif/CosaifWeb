    ...
    >
      {report.activeTab === "rankings" && (
        <>
          ...
          <MaquinistasRankingSection operadores={report.rawReport?.rankingOperadores ?? []} />
        </>
      )}

      {report.activeTab === "overview" && (
        <MaquinistasAgrupadosSection
          porEmpresa={report.rawReport?.operadoresPorEmpresa ?? []}
          porLocalidad={report.rawReport?.operadoresPorLocalidad ?? []}
        />
      )}
    </ReportLayout>
  );
