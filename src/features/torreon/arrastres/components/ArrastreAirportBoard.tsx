...
import { AlertTriangle, Boxes, MapPin, Radio } from "lucide-react";
      ...
      <div className="flex flex-wrap items-center justify-between gap-3 border-b border-slate-200 bg-slate-100 px-4 py-3 dark:border-slate-800 dark:bg-slate-950">
        <div>
          <div className="flex items-center gap-2 font-mono text-[11px] font-black uppercase tracking-[0.22em] text-emerald-700 dark:text-emerald-300">
            ...
            Terminal de arrastres
          </div>
          <h3 className="mt-0.5 text-base font-black text-slate-950 dark:text-white">Conjuntos en cola operativa</h3>
        </div>
...
function StatusLabel({ status }: { status: string }) {
  ...
  return <span className={`inline-flex rounded-md border px-2 py-1 text-xs font-black ${tone}`}>{status.replace(/_/g, " ")}</span>;
}
