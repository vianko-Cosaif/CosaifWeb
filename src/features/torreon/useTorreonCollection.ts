...
export async function readTorreonJson<T>(
  url: string,
  signal: AbortSignal,
  force = false,
): Promise<T> {
  const payload = await cachedFetchJson<T>(
    url,
    {
      credentials: "include",
      cache: "no-store",
  ...
  if (payload && typeof payload === "object" && "success" in payload && payload.success === false) {
    ...
    throw new Error(
      failure.message || failure.error || "No se pudo consultar la operación de Torreón.",
    );
  }
  return payload;
}
...
export type TorreonPage<T> = { data: T[]; meta: TorreonPageMeta };
...
export function useTorreonCollection<T>({
  ...
  queryKey: string;
  enabled?: boolean;
  fetchRows: (signal: AbortSignal, force: boolean) => Promise<T[] | TorreonPage<T>>;
}) {
  ...
  const load = useCallback(
    (force = false): Promise<void> => {
      if (!enabled || !context.current.active || context.current.key !== queryKey)
      ...
      const current = () =>
        ...
        context.current.key === queryKey &&
        pending.current === task;
      setState((previous) => ({
        key: queryKey,
        rows: previous.key === queryKey ? previous.rows : [],
        loaded: previous.key === queryKey && previous.loaded,
        loading: true,
        error: null,
        meta: previous.key === queryKey ? previous.meta : undefined,
      }));
      task.promise = (async () => {
        let forceRead = force;
        try {
          do {
            ...
            const result = await fetchRows(controller.signal, forceRead);
            const rows = Array.isArray(result) ? result : result.data;
            const meta = Array.isArray(result) ? undefined : result.meta;
            if (!current()) return;
            if (revision === rowsRevision.current) {
              setState({ key: queryKey, rows, meta, loaded: true, loading: true, error: null });
            } else {
          ...
          if (current())
            setState((previous) => ({
              ...previous,
              error: error instanceof Error ? error.message : "No se pudo actualizar la operación.",
            }));
        } finally {
          if (current()) {
  ...
  const refreshItem = useCallback(
    async (
      ...
      try {
        ...
        if (!controller.signal.aborted && context.current === scope && scope.active)
          await load(true);
      } finally {
        if (itemRequests.current.get(id) === controller) itemRequests.current.delete(id);
        if (context.current === scope && scope.active)
          setItemLoading(itemRequests.current.size > 0);
      }
    },
  ...
  return {
    ...
    meta: visible ? state.meta : undefined,
    setRows,
