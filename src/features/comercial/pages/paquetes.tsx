"use client";
import { loadCompleteAnalytics } from "@/features/comercial/lib/analytics";
...
import type { AnalyticsSummary, CommercialOrigin, Contract } from "../types";
import { useCommercialData } from "../components/CommercialDataProvider";
import { EmptyPanel, Field, LoadingPanel, ModuleHeader, Notice } from "../components/CommercialUi";
import { buildQuery } from "../lib/api";
import responsive from "../components/CommercialRecords.module.css";
import { formatNumber } from "../lib/format";
import { useCrmCatalog } from "../lib/useCrmList";
...
export default function ContractedMovementsPage() {
  ...
  const { items: contracts, loading: contractsLoading, error: contractsError } = useCrmCatalog<Contract>("/bff/comercial/contratos?pageSize=100");
  ...
  async function runReport() {
    ...
    try {
      ...
      const data = await Promise.all(months.map(async (monthKey) => {
        ...
        const monthAnalytics = await loadCompleteAnalytics(`/bff/comercial/analitica?${query}`, { signal: controller.signal });
  ...
  return <div className={`${responsive.container} space-y-5`}>
    ...
    <section className="commercial-card overflow-hidden">
      <header className="flex flex-col gap-3 border-b border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-5 sm:flex-row sm:items-center sm:justify-between">
        <div><p className="commercial-label">Filtros de consulta</p><h2 className="break-words text-lg font-semibold tabular-nums text-[var(--app-text)]">Periodo, clientes y contratos</h2></div>
      ...
      {filtersOpen ? <div id="contract-report-filters" className="grid gap-5 p-5 xl:grid-cols-2">
        ...
        <section className="rounded-2xl border border-[var(--app-border)] p-4">
          ...
          <label className="relative mt-4 block"><Search className="pointer-events-none absolute left-3 top-1/2 h-4 w-4 -translate-y-1/2 text-[var(--app-text-muted)]"/><input value={clientSearch} onChange={(event) => setClientSearch(event.target.value)} className="commercial-input pl-10" aria-label="Buscar cliente con contratos" placeholder="Buscar cliente…"/></label>
...
function ContractSection({ group, localidadId, origin }: { group: ContractGroup; localidadId?: number; origin?: CommercialOrigin }) {
  ...
  return <article data-contract-id={group.contract.id} className={`commercial-card overflow-hidden border-l-4 ${clientStyle.border}`}>
    <header className={`${responsive.groupHeader} border-b border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-4 sm:p-5`}>
      <div className="min-w-0"><span className={`inline-flex max-w-full items-center gap-2 rounded-full px-3 py-1 text-[10px] font-black uppercase tracking-[.1em] ${clientStyle.badge}`}><Building2 className="h-3.5 w-3.5 shrink-0"/><span className="truncate">{group.contract.cliente?.empresaNombre || "Cliente"}</span></span><h2 className="mt-3 break-words text-xl font-semibold text-[var(--app-text)]">{group.contract.nombre}</h2><p className="mt-1 text-xs font-bold text-[var(--app-text-muted)]">{group.contract.folio} · {months.length} {months.length === 1 ? "mes revisado" : "meses revisados"} · {group.contract.estado}</p></div>
...
function OperationRow({ card, contractId, monthKey, localidadId, selectedOrigin }: { card: ContractMonthCard; contractId: number; monthKey: string; localidadId?: number; selectedOrigin?: CommercialOrigin }) {
  ...
  return <div className={`rounded-xl border p-3 ${style.border} ${style.surface}`}>
    <div className={responsive.operationSummary}><div className="min-w-0"><span className={`inline-flex items-center gap-2 rounded-full px-2.5 py-1 text-[10px] font-black ${style.badge}`}><i className={`h-2 w-2 rounded-full ${style.dot}`}/>{style.label}</span><p className="mt-2 text-xs font-bold text-[var(--app-text-muted)]">{card.rules.map(({ rule }) => operationLabel(rule)).join(" · ")}</p></div><div className={responsive.operationValues}><div className="grid min-w-0 grid-cols-3 gap-2 text-right"><TinyMetric label="Contratado" value={card.contracted == null ? "Sin límite" : formatNumber(card.contracted)}/><TinyMetric label="Consumido" value={formatNumber(card.used)}/><TinyMetric label="Excedente" value={formatNumber(card.excess)}/></div><button type="button" data-testid={`contract-card-download-${contractId}-${monthKey}-${card.category}`} onClick={downloadCard} disabled={downloadState === "loading"} className="commercial-secondary min-h-11 shrink-0 px-3 text-xs">{downloadState === "loading" ? <Loader2 className="h-4 w-4 animate-spin"/> : <Download className="h-4 w-4"/>}{downloadState === "loading" ? "Construyendo…" : `Descargar ${style.label}`}</button></div></div>
...
function CompactMetric({ label, value }: { label: string; value: number }) {
  return <div className="min-w-0 rounded-xl border border-[var(--app-border)] bg-[var(--app-surface)] px-3 py-2 text-right"><p className="break-words text-lg font-semibold tabular-nums text-[var(--app-text)]">{formatNumber(value)}</p><p className="text-[9px] font-black uppercase tracking-[.08em] text-[var(--app-text-muted)]">{label}</p></div>;
}

function TinyMetric({ label, value }: { label: string; value: string }) {
  return <div><p className="break-words text-sm font-semibold tabular-nums text-[var(--app-text)]">{value}</p><p className="text-[9px] font-black uppercase tracking-[.06em] text-[var(--app-text-muted)]">{label}</p></div>;
}
