...
import type { AnalyticsSummary, BillingCut, Contract, CrmClient } from "../types";
import { buildQuery, commercialApi } from "../lib/api";
import { formatDate, formatMoney, formatNumber, humanize, todayIso } from "../lib/format";
import { useCrmCatalog } from "../lib/useCrmList";
import { useCommercialData } from "../components/CommercialDataProvider";
import { EmptyPanel, Field, LoadingPanel, MetricCard, Modal, ModuleHeader, Notice } from "../components/CommercialUi";
...
export default function CollectionsPage() {
  ...
  const { items: contracts } = useCrmCatalog<Contract>("/bff/comercial/contratos?pageSize=100");
  ...
  const { items: cuts, loading: cutsLoading, error, reload } = useCrmCatalog<BillingCut>(`/bff/comercial/cobranza/cortes?${cutQuery}`);
  ...
  useEffect(() => {
    ...
    Promise.all(sortedMonths.map(async (month) => {
      ...
      return [month, await loadCompleteAnalytics(`/bff/comercial/analitica?${query}`, { signal: controller.signal })] as const;
  ...
  return <div className="space-y-5">
    <ModuleHeader eyebrow="Control contractual" title="Cortes por contrato" description="Compare uno o varios meses siguiendo la ruta tipo de contrato → contrato → mes." icon={HandCoins} actions={<button type="button" disabled={!billingRows.length || generalDownloading} onClick={downloadGeneralCut} className="commercial-secondary"><FileSpreadsheet className="h-4 w-4"/>{generalDownloading ? "Generando…" : "Corte general"}</button>}/>
...
function CutReviewPanel({ row, analytics, localityId, origin, onCreate, onPay, onEdit, onInvoice, onChanged }: {
  ...
  const evidenceBody = useMemo(() => ({
    ...
    origin,
  }), [row.contractId, row.cut?.id, row.periodStart, row.periodEnd, analytics.meta.period, analytics.meta.referenceDate, localityId, origin]);
