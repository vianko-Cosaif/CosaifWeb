...
export default function CommercialPeriodBar({ showCompany = true, showLocality = true, showOrigin = false, showPeriod = true }: { showCompany?: boolean; showLocality?: boolean; showOrigin?: boolean; showPeriod?: boolean }) {
  const { analytics, catalogs, filters, loading, setFilters } = useCommercialData();
  const clientName = filters.empresaId ? catalogs?.companies.find((item) => item.id === filters.empresaId)?.nombre : "Todos los clientes";
  const localityName = filters.localidadId ? catalogs?.localities.find((item) => item.id === filters.localidadId)?.nombre : "Todos los patios";
  ...
  function changeLocality(raw: string) {
    ...
    const name = localidadId ? catalogs?.localities.find((item) => item.id === localidadId)?.nombre : undefined;
  ...
  return <section className="commercial-card p-4 sm:p-5">
    ...
    <div className={`mt-4 grid gap-4 ${showPeriod ? "xl:grid-cols-[minmax(0,.9fr)_minmax(0,1.4fr)]" : ""}`}>
      {showPeriod ? <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-4">
        ...
        <div className="grid grid-cols-2 gap-1.5 rounded-xl bg-[var(--app-surface-muted)] p-1.5 sm:grid-cols-5">{PERIOD_OPTIONS.map((item) => <button key={item.value} type="button" aria-pressed={filters.period === item.value} onClick={() => setFilters({ period: item.value })} className={`min-h-10 rounded-lg px-2 text-xs font-black transition ${filters.period === item.value ? "bg-slate-950 text-white shadow-sm dark:bg-white dark:text-slate-950" : "text-[var(--app-text-muted)] hover:bg-[var(--app-surface)]"}`}>{item.short}</button>)}</div>
      ...
      <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-4">
        ...
        <div className={`mt-3 grid gap-3 ${showOriginSelector ? "md:grid-cols-3" : "md:grid-cols-2"}`}>
          {showCompany ? <label><span className="commercial-label">Cliente</span><select value={filters.empresaId ?? ""} onChange={(event) => setFilters({ empresaId: event.target.value ? Number(event.target.value) : undefined })} className="commercial-select"><option value="">Todos los clientes</option>{catalogs?.companies.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label> : null}
          {showLocality ? <label><span className="commercial-label"><MapPin className="mr-1 inline h-3.5 w-3.5"/>Patio</span><select value={filters.localidadId ?? ""} onChange={(event) => changeLocality(event.target.value)} className="commercial-select"><option value="">Todos los patios</option>{catalogs?.localities.map((item) => <option key={item.id} value={item.id}>{item.nombre}</option>)}</select></label> : null}
...
function PeriodSelector({ period, referenceDate, onChange }: { period: CommercialPeriod; referenceDate: string; onChange: (value: string) => void }) {
  ...
  if (period === "BIMONTH") {
    ...
    return <div className="grid grid-cols-[minmax(0,.65fr)_minmax(0,1.35fr)] gap-2"><YearSelect value={year} onChange={(nextYear) => onChange(periodDate(nextYear, (bimonth - 1) * 2 + 1))}/><label><span className="commercial-label">Bimestre</span><select value={bimonth} onChange={(event) => onChange(periodDate(year, (Number(event.target.value) - 1) * 2 + 1))} className="commercial-select">{BIMONTHS.map((item) => <option key={item.value} value={item.value}>{item.label}</option>)}</select></label></div>;
  }
  if (period === "SEMESTER") {
    const semester = month <= 6 ? 1 : 2;
    return <div className="grid grid-cols-[minmax(0,.65fr)_minmax(0,1.35fr)] gap-2"><YearSelect value={year} onChange={(nextYear) => onChange(periodDate(nextYear, semester === 1 ? 1 : 7))}/><label><span className="commercial-label">Semestre</span><select value={semester} onChange={(event) => onChange(periodDate(year, Number(event.target.value) === 1 ? 1 : 7))} className="commercial-select"><option value={1}>1 · Enero – junio</option><option value={2}>2 · Julio – diciembre</option></select></label></div>;
  }
