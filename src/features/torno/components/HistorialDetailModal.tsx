...
export default function HistorialDetailModal({
  ...
  const handleDownloadPdf = () => {
    ...
    const original = item.original && typeof item.original === "object" ? (item.original as Record<string, unknown>) : {};
    const record = (value: unknown): Record<string, unknown> => value && typeof value === "object" ? value as Record<string, unknown> : {};
    const movimiento = record(original.movimiento);
    const movimientoId = original.movimientoId ?? movimiento.id ?? movimiento.movimientoId ?? item.id;
    const companyName =
      record(original.empresa).nombre ??
      record(original.company).name ??
      original.companyName ??
      original.empresaNombre ??
      record(movimiento.empresa).nombre ??
      record(record(record(original.ruedaSolicitud).movimiento).empresa).nombre ??
      "";
    ...
    try {
      ...
      downloadTornoHistoryPdf({
        ...
        movimientoId: typeof movimientoId === "number" || typeof movimientoId === "string" ? movimientoId : item.id,
      ...
      <div className="grid gap-3 p-3 xl:grid-cols-[minmax(0,1fr)_320px]">
        ...
        <div className="rounded-lg border border-slate-200 bg-white p-3 dark:border-slate-800 dark:bg-slate-950">
          ...
          <h4 className="mt-1 text-lg font-black text-slate-950 dark:text-slate-100">
            {selectedPosition ? `Rueda ${selectedPosition}` : "Sin selección"}
          </h4>
