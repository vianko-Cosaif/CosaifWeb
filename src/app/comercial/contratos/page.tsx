...
export default function ContractsPage() {
  ...
  return <div className="space-y-5">
    <ModuleHeader eyebrow="Ambiente de contratos" title="Contratos, vigencias y reglas" description="Registre el acuerdo general por cliente. Las cantidades, periodicidades, patios y estados que cuentan se definen después en Reglas y paquetes." icon={FileText} actions={<button type="button" className="commercial-primary" onClick={() => setCreating(true)} disabled={!clients.length}><Plus className="h-4 w-4"/>Nuevo contrato</button>}/>
    ...
    {loading ? <LoadingPanel text="Cargando contratos…"/> : <>
      ...
      <section className="commercial-card overflow-hidden"><div className="overflow-x-auto"><table className="w-full min-w-[1050px] text-left text-sm"><thead className="bg-[var(--app-surface-muted)] text-[10px] font-black uppercase tracking-[.1em] text-[var(--app-text-muted)]"><tr><th className="px-5 py-3">Cliente</th><th className="px-4 py-3">Contrato</th><th className="px-4 py-3">Vigencia</th><th className="px-4 py-3">Control del consumo</th><th className="px-4 py-3">Orden de compra</th><th className="px-4 py-3">Día de corte</th><th className="px-4 py-3">Reglas</th><th className="px-4 py-3">Estado</th></tr></thead><tbody className="divide-y divide-[var(--app-border)]">{contracts.map((contract) => <tr key={contract.id}><td className="px-5 py-4 font-black text-[var(--app-text)]">{contract.cliente?.empresaNombre || "—"}</td><td className="px-4 py-4"><p className="font-black text-[var(--app-text)]">{contract.nombre}</p><p className="text-xs text-[var(--app-text-muted)]">{contract.folio}</p></td><td className="px-4 py-4 text-xs font-bold text-[var(--app-text-muted)]">{formatDate(contract.fechaInicio)} – {formatDate(contract.fechaFin)}</td><td className="px-4 py-4"><ContractControl contract={contract}/></td><td className="px-4 py-4 font-bold text-[var(--app-text)]">{contract.ordenCompra || "Pendiente"}</td><td className="px-4 py-4 font-black text-[var(--app-text)]">{contract.diaCorte ? `Día ${contract.diaCorte}` : "Sin día fijo"}</td><td className="px-4 py-4"><p className="font-black text-[var(--app-text)]">{contract._count?.paquetes || 0}</p><p className="text-xs text-[var(--app-text-muted)]">paquetes configurados</p></td><td className="px-4 py-4"><StateBadge value={contract.estado}/></td></tr>)}</tbody></table></div>{!contracts.length ? <div className="p-5"><EmptyPanel title="No hay contratos en esta vista" text="Registre el primer contrato o cambie los filtros."/></div> : null}</section>
    </>}
