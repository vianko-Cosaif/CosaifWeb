...
export default function CommercialPeriodBar({ showCompany = true, showLocality = true, showOrigin = false }: { showCompany?: boolean; showLocality?: boolean; showOrigin?: boolean }) {
  ...
  const originName = !arrastreAvailable || filters.origin === "NATURAL" ? "Naturales" : filters.origin === "ARRASTRE" ? "Arrastre" : "Naturales y Arrastre";
  function changeLocality(raw: string) {
    ...
    setFilters({ localidadId, ...localidadId && !isTorreon(name) ? { origin: "NATURAL" as const } : {} });
  }
  return <section className="commercial-card p-4 sm:p-5">
    ...
    <div className="mt-4 grid gap-4 xl:grid-cols-[minmax(360px_.9fr)_minmax(520px_1.4fr)]">
      ...
      <div className="rounded-2xl border border-[var(--app-border)] bg-[var(--app-surface-subtle)] p-4">
        ...
        <div className={`mt-3 grid gap-3 ${showOriginSelector ? "md:grid-cols-3" : "md:grid-cols-2"}`}>
          ...
          {showOriginSelector ? <label><span className="commercial-label"><TrainFront className="mr-1 inline h-3.5 w-3.5"/>Operación</span><select value={filters.origin ?? ""} onChange={(event) => setFilters({ origin: event.target.value ? event.target.value as "NATURAL" | "ARRASTRE" : undefined })} className="commercial-select"><option value="">Naturales y Arrastre</option><option value="NATURAL">Solo Naturales</option><option value="ARRASTRE">Solo Arrastre</option></select></label> : null}
        </div>
      </div>
    </div>
    <p className="mt-4 text-xs font-bold text-[var(--app-text-muted)]"><span className="text-[var(--app-text)]">{analytics?.meta.periodLabel || "Periodo seleccionado"}</span><span aria-hidden="true"> · </span>{clientName}<span aria-hidden="true"> · </span>{localityName}{showOrigin ? <><span aria-hidden="true"> · </span>{originName}</> : null}</p>
  </section>;
}
