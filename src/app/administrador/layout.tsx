...
// Si algún día quieres monitoreo global desde admin, actívalo:
...
export default function AdminLayout({ children }: { children: React.ReactNode }) {
  ...
  useEffect(() => {
    // Rol desde cookie, forzamos ADMINISTRADOR si no coincide
    const roleRaw = (getClientCookie("role") ?? "").toUpperCase();
    setRol(roleRaw === "ADMINISTRADOR" ? "ADMINISTRADOR" : "ADMINISTRADOR");

    // Localidad opcional para mostrar contexto; si no hay, queda "Global"
    const locId = getClientCookie("locId");
    setLocLabel(locId ? `Loc ${locId}` : "Global");

    // Empresa del usuario si aplica
      ...
      <div className="relative z-10 flex min-h-svh flex-1 flex-col">
        ...
        <header className="sticky top-[max(0px,env(safe-area-inset-top))] z-20 border-b border-slate-200/70 bg-white/80 backdrop-blur supports-[backdrop-filter]:bg-white/60 dark:border-slate-700/60 dark:bg-slate-900/70">
          <div
            ...
            style={{
              paddingLeft: "max(3.75rem, calc(env(safe-area-inset-left) + 0rem))",
              paddingRight: "max(1rem, env(safe-area-inset-right))",
            }}
          >
            <h1 className="text-base font-semibold tracking-tight sm:text-lg">Panel de Administración</h1>
